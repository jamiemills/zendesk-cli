...
class ZendeskAdapter(BaseAdapter):
    ...
    def _get_satisfaction_ratings(self, client: ZendeskClient, ticket_id: str) -> Dict[str, Any]:
        """Get satisfaction ratings for a ticket (Zendesk-specific)."""
        try:
            response = client._make_request("GET", f"tickets/{ticket_id}/satisfaction_rating.json")
            return response.get("satisfaction_rating", {})
        except Exception:
            return {}
    
    def _get_ticket_metrics(self, client: ZendeskClient, ticket_id: str) -> Dict[str, Any]:
    ...
    def _search_advanced(self, client: ZendeskClient, query: str, sort_by: str = None, sort_order: str = None) -> List[Dict[str, Any]]:
